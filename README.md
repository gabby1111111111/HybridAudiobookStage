...
- 三种朗读模式：
  ...
  - 只读角色台词；
  - 全文使用一个声音。
- 多 Provider Profile：Edge、IndexTTS2 / OpenAI 兼容、豆包原生、MiniMax 原生和小米 MiMo 原生。
...
- 可选“AI 回复完成后自动播放”，默认关闭，只播放当前聊天最新回复。
...
- **角色台词语速**：只调整角色台词的播放速度，不改变音频缓存；
- **旁白语速**：只调整旁白的播放速度，并在全文队列切换旁白段落时持续生效；
- **音量**：只改变播放音量，不改变音频缓存；
- **AI 回复完成后自动播放**：默认关闭；开启后，角色最新回复渲染完成会自动进入普通有声书播放队列；
...
1. 在“要测试的声音服务”选择内置的“MiniMax 原生”。
2. 填写 API Key，并选择国内站 `minimaxi.com` 或国际站 `minimax.io`。
3. 模型下拉框提供 `speech-2.8-hd`、`speech-2.8-turbo`、`speech-2.6-hd`、`speech-2.6-turbo`、`speech-02-hd`、`speech-02-turbo`，也可以选择“自定义模型”。
4. 填写 API Key 后点击“刷新官方音色”，可加载系统音色、自己的复刻音色和自己的生成音色；仍可通过“手动添加 Voice ID”使用目录外音色。
5. 测试区、旁白、角色默认和全文路由共用同一套可搜索音色列表。“测试连接”只检查配置完整；“试听声音”才会真实调用 MiniMax。

MiniMax 请求由服务器插件发送到固定的 `/v1/t2a_v2`，前端不会直接访问云端。API Key 不进入缓存 Key、Audit、共享缓存元数据或错误回包。

### 小米 MiMo 原生 TTS

1. 在“要测试的声音服务”选择内置的“小米 MiMo 原生”。
2. 填写 API Key 并选择模型。首版开放 `mimo-v2.5-tts` 普通 TTS；音色设计和音色复刻模型会显示为“首版暂未开放”，不能误选。
3. 音色下拉框内置 `mimo_default`、冰糖、茉莉、苏打、白桦、Mia、Chloe、Milo、Dean 九个预置音色，也可以手动添加 MiMo `audio.voice` 接受的字符串；测试区和所有路由共用该列表。
4. 点击“试听声音”完成真实账号、模型和音色验证。

当前轻量首版支持九个预置音色和自定义音色字符串；音色设计、音色复刻模型只作明确的未开放提示，不保存克隆参考音频，也不包含 Key 池、复杂音效或卡片式管理页面。
