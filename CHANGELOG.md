...
- 新增 MiniMax 原生与小米 MiMo 原生 Provider；与豆包一起通过 SillyTavern 服务器代理接入轻量 TTS，并复用统一路由、播放器与多层音频缓存。
- MiniMax 新增六个内置模型、自定义模型和官方音色刷新；小米 MiMo 新增三个模型状态与九个预置音色。测试试听、旁白、角色默认和全文路由统一复用可搜索音色列表。
- 新增默认关闭的“AI 回复完成后自动播放”开关；只播放当前聊天最新的角色回复，并避免刷新历史消息时重复触发。

### 修复

- 修复 MiniMax 与小米 MiMo 在前端已更新、当前 SillyTavern 仍运行旧版服务端助手时只得到含糊 404 的问题；连接测试现在会明确提示同步 `server-plugin` 并重启当前酒馆实例。
- 修复正常的服务器音频缓存未命中反复以 GET 404 污染浏览器控制台的问题；读取完整音频前会先调用轻量存在性检查。
- 补充 GitHub 安装用户的双组件安装、升级和多实例排障说明，明确原生云 TTS 与服务器共享缓存必须单独安装服务器插件。
- 修复切换 Profile、音色、特定角色覆盖或合成语速后，单句按钮仍继续播放旧音色会话的问题；现会保留各音色缓存，并按当前合成设置重新命中对应缓存。
- 将原“合成语速 / 播放倍速”拆成“角色台词语速 / 旁白语速”，全文队列会按当前段类型持续应用对应速度。
- 修复选中半句或部分段落时误走迁移遗留的 IndexTTS2“全文声音”并请求通用代理的问题；混合模式会识别所属台词或旁白，无法归类时改用当前角色默认声音。
- 修复聊天气泡等外层包装中的完整 TTS 台词未被识别、外层昵称可能被当作旁白朗读的问题。
